...
This project implements a CNN pipeline in PyTorch for classifying hand gestures into three classes: Rock, Paper, and Scissors.
...
- Tested on 10 custom smartphone photographs to check real-world generalization.

The whole pipeline (dataset download, preprocessing, training, evaluation, and inference) runs automatically in Google Colab with a single "Run All". No manual file uploads or path changes needed.
...
The model is trained and evaluated on the publicly available synthetic RPS dataset. It contains CGI-rendered images of hands making the three gestures against a plain background, at varying angles, skin tones, and lighting conditions.
...
To test real-world generalization, 10 smartphone photographs were taken of my hand performing the three gestures against a plain wall:
...
All images are resized to 150x150 pixels and normalized using ImageNet mean/std statistics.

### Training Transforms

| # | Transform | Details |
|:--|:----------|:--------|
| 1 | `Resize(150, 150)` | Scale all images to uniform 150x150 |
| 2 | `RandomHorizontalFlip(p=0.5)` | 50% chance of horizontal flip |
| 3 | `RandomRotation(10)` | Random rotation between -10 and +10 degrees |
| 4 | `ColorJitter(brightness=0.1, contrast=0.1)` | Slight brightness/contrast variation |
| 5 | `ToTensor()` | Convert PIL image to tensor, scale [0, 255] to [0, 1] |
...
| # | Transform | Details |
|:--|:----------|:--------|
| 1 | `Resize(150, 150)` | Scale to 150x150 |
| 2 | `ToTensor()` | Convert to tensor |
| 3 | `Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])` | ImageNet normalization |

> **Note:** Augmentation is only applied during training. Test/inference uses deterministic transforms for consistent evaluation.

Data is loaded using `torchvision.datasets.ImageFolder` (auto-assigns labels from folder names) and wrapped in `torch.utils.data.DataLoader` with `batch_size=64`.
...
+------------------------------------------+
|  CLASSIFIER HEAD                         |
|  AdaptiveAvgPool2d(4x4)  -> 128 x 4 x 4 |
|  Flatten                 -> 2048         |
|  Dropout(0.4)                            |
|  Linear(2048 -> 256)                     |
|  ReLU                                    |
...
| Block | Layer | Input Ch | Output Ch | Kernel | Padding | Output Size |
|:------|:------|:--------:|:---------:|:------:|:-------:|:-----------:|
| Conv Block 1 | Conv2d + BN + ReLU + MaxPool | 3 | 32 | 3x3 | 1 | 32 x 75 x 75 |
| Conv Block 2 | Conv2d + BN + ReLU + MaxPool | 32 | 64 | 3x3 | 1 | 64 x 37 x 37 |
| Conv Block 3 | Conv2d + BN + ReLU + MaxPool | 64 | 128 | 3x3 | 1 | 128 x 18 x 18 |
| Adaptive Pool | AdaptiveAvgPool2d | 128 | 128 | - | - | 128 x 4 x 4 |
| Flatten | - | - | - | - | - | 2,048 |
...
| Choice | Rationale |
|:-------|:----------|
| 3x3 kernels with padding=1 | Preserves spatial dimensions before pooling, captures local patterns |
| BatchNorm after Conv | Stabilizes training, allows higher learning rates, mild regularization |
| ReLU activation | Avoids vanishing gradient problem, computationally cheap |
| MaxPool2d(2x2) | Halves spatial dimensions, gives translation invariance, reduces computation |
| AdaptiveAvgPool2d(4x4) | Fixed-size output regardless of input resolution |
...
- Training loss went down steadily from 0.62 to 0.03, and training accuracy crossed 99% by epoch 6.
- Validation metrics fluctuated at epochs 3 (val loss 1.10) and 7 (val loss 1.08). This is probably because:
  - The validation set is small (372 images), so batch statistics are unstable.
  - BatchNorm behaves differently between train and eval mode.
  - Adam's adaptive learning rate can sometimes make aggressive updates.
- The model recovered each time and reached its best validation accuracy (95.43%) at epoch 10.

---

## Evaluation on Standard Test Set

On the held-out RPS test set (372 images), the model got an overall accuracy of **95%**.
...
- **Paper:** 107/124 correct, 86% recall, the weakest class.
...
Paper has high precision (1.00) but lower recall (0.86), meaning when the model predicts "paper", it's always correct, but it misses some actual paper images and labels them as other classes.
...
- This makes the silhouette look similar to a scissors or rock hand at certain angles.

---

## Real-World Prediction on Custom Photos

The trained model was run on all 10 custom smartphone photographs:
...
- All 4 rock images were correctly identified with 100% confidence.
- All 3 paper images were incorrectly classified as rock (97-100% confidence).
- All 3 scissors images were incorrectly classified as rock (100% confidence).

The model basically predicted "rock" for every image, so only the actual rock photos ended up being correct.

---

## Domain Gap Analysis

The accuracy dropped from 95% on the standard test set to just 40% on the custom phone photos. This is a clear case of domain shift / distribution shift.
...
The model was trained from scratch with only 619K parameters on 2,520 synthetic images. Instead of learning actual hand-shape features, it probably picked up on low-level dataset-specific cues like:
...
A closed fist (rock) is a visually simple, compact shape and the most distinct of the three gestures. This basic shape was still recognizable even across different domains. Paper and scissors need more detailed finger recognition that the small from-scratch model couldn't generalize.
...
|:------------|:----------------|
| Transfer learning - use pretrained ResNet18/MobileNet backbone (ImageNet weights), fine-tune only the classifier head | Model would start with general visual features instead of learning from random initialization |
...
| Domain adaptation / fine-tuning - fine-tune on a small number of real photographs (5-10 per class) | Adapts the decision boundary to the real-world domain |
| Diverse training data - mix real-photo RPS datasets with the CGI-rendered one | Better matches the deployment distribution |
| Larger model - use deeper architecture or more parameters | More capacity to learn complex, generalizable features |
...
<p align="center">
  CSE 3202 - Artificial Intelligence and Machine Learning Lab<br/>
